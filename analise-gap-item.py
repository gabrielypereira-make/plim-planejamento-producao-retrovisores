...
# 2. RESOLUÇÃO 
# ============================================================
for gap_da_vez in GAPS_PARA_TESTAR:
    print(f"\n>>> PROCESSANDO GAP: {gap_da_vez:.2f}")
    ...
    for t_hoje in T_planejamento:
        ...
        status_f2 = prob.solve(PULP_CBC_CMD(msg=0, gapRel=gap_da_vez))
        ...
        for i in P:
            ...
            # FILTRO: Valores abaixo de 0.001 são ignorados (limpa o 0,0006)
            ...
            # ATUALIZAÇÃO PARA O PRÓXIMO DIA (Limpando ruído)
...
# 3. EXPORTAÇÃO
# ============================================================
pd.DataFrame(log_por_item).to_excel("analise_gap_item.xlsx", index=False)
print(f"\n✔ Sucesso! Arquivo gerado: analise_gap_item.xlsx")
